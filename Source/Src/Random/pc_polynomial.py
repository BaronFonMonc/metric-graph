...
from Random.graph_utils import Edge, Graph
...
def build(n: int, m: int, g: Graph) -> sympy.core.Expr:
    ...
    for sub_g in get_all_subg(g, s):
        ...
        for v in sub_g.g:
            ...
            for cs in all_routes(sub_g, s, v):
                Help = T - sum(c * edge.t for edge, c in cs)
    ...
    #print(R1,R2,sep='\n')
    print(sympy.simplify(sympy.Poly(R1, T)).as_expr())
    print(sympy.simplify(sympy.Poly(R2, T)).as_expr())
