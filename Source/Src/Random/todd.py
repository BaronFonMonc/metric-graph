...
#from todd_precalc import pre_calc
...
def build_rk_new(k: int, l: Union[float, int, sym.Symbol], ws: list[Union[float, sym.Symbol]]):
    p1 = 1
    for wi in ws:
        p1 /= wi
    #s1 = 0
    to_add_1 = l ** k / factorial(k)
    to_add_2 = (sum(ws) / 2) * l ** (k-1) / factorial(k-1)
    #for s in range(k + 1):
    #    to_add = l ** s / factorial(s) * td(k - s, ws)
    #    s1 += to_add
    return p1 * (to_add_1 + to_add_2)
